...
EXPECTED_DELIVERY_RUNTIME_SCHEMA_VERSION = "2026-09-16-il-partner-transfer-v25"
...
def get_stage2_report_sheet_names(destination_type="全部"):
    if destination_type == "FBA":
        return ["FBA仓点分析", "FBA派送方式分析", "货量", "FBA货量排行", "发车量", "派送时效", "调拨数据", "每方价格参考", "分类型价格参考", "黄金标准数据", "派送二_匹配后批次数据", "派送二_车次汇总核对", "邮编异常审核", "区域识别规则", "干线识别规则"]
    ...
    return ["FBA仓点分析", "FBA派送方式分析", "货量", "FBA货量排行", "FBX平台仓货量", "发车量", "派送时效", "调拨数据", "每方价格参考", "分类型价格参考", "黄金标准数据", "派送二_匹配后批次数据", "派送二_车次汇总核对", "邮编异常审核", "区域识别规则", "干线识别规则"]
...
def build_stage2_report_for_destination(cleaned_batches, match_df=None, period_type="按周统计", destination_type="全部"):
    all_matched = delivery_workflow.prepare_stage2_for_report(cleaned_batches, match_df, period_type)
    ...
    if destination_type in ["全部", "FBA"]:
        report["FBA仓点分析"] = delivery_match_adapter._safe_round(fba_summary, "成本")
        report["FBA派送方式分析"] = delivery_match_adapter._safe_round(fba_methods, "成本")
...
st.caption(
    ...
    "LA至IL合作仓（610 Supreme Dr, Bensenville, IL 60106）按LA-IL列入调拨数据，目的地仍为FBX。仅本批次明确标记IL合作仓，或门牌街道及城市/邮编吻合时识别；不能仅凭IL州、60106邮编或同车次备注认定。沿用FTL调拨统计和有效整车成本样本规则。"
    ...
    "运输类型先按源数据判断，再按仓库和真实车次校验：同车次同时含FTL和LTL时整车统一按FTL；同车次只有LTL但总出库体积大于60CBM时按FTL，60CBM仍按LTL。派送卡车/承运商名称（包括AMAZON FREIGHT）不再触发FTL重判。"
        ...
        try:
            if not selection_complete:
                ...
                output = tool_common.write_sheets_to_excel({
                    ...
                    "车次汇总核对": delivery_workflow.build_trip_audit(cleaned_batches),
                    "邮编异常数据": zip_audit_df,
                    "无效数据": invalid_detail,
                })
    ...
    if uploaded_files:
        try:
            ...
            if st.button("开始分析", type="primary", key="run_normal_analysis"):
                if not selection_complete:
                    st.warning("请先把仓点、分析模块、统计周期都选择完整。")
                elif period_type != ORIGINAL_FILE_PERIOD and not selected_date_range_is_valid(date_range):
