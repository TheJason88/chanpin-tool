...
if analysis_module == DELIVERY_STAGE1_MODULE:
    ...
    if period_type == ORIGINAL_FILE_PERIOD:
        st.info("派送二按原文件时间范围：货量、发车和时效按批次出库时间范围归期；每方价格、分类型价格和调拨成本按批次创建时间范围归期。")
    else:
        st.info("派送数据匹配及分析会先完成邮编/平台仓匹配，再按目的地类型生成报告。货量、发车和时效按批次出库时间归期，所有派送成本按批次创建时间归期。成本输出为“每方价格参考”和“分类型价格参考”；其中LTL不显示整车价格。")
...
st.caption(
    ...
    "派送二成本输出为每方价格参考和分类型价格参考，并统一按批次创建时间归期；前者按FBA/FBX目的地仓点汇总，后者合并大车地板、大车卡板、小车和LTL。只有原始派送成本大于0的批次进入成本的方数分母和成本分子；零成本批次不稀释每方价格。FTL整车价格只使用有真实车次和精确车份额的数据，LTL及缺车次批次的整车价格留空。批次基础派送成本不按整车总成本二次分摊；FTL大车地板另按批次精确车份额×$200增加装车费。"
